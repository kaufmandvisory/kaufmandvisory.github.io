...
export const SOURCE_REGISTRY = Object.freeze([
  ...
  { id: 'pt_cirs_10', jurisdiction: 'portugal', authority: 'Autoridade Tributária e Aduaneira', title: 'Código do IRS · artigo 10.º', url: 'https://info.portaldasfinancas.gov.pt/pt/informacao_fiscal/codigos_tributarios/cirs_rep/Pages/irs10.aspx', source_type: 'CONSOLIDATED_LAW', binding_level: 'PRIMARY_LAW', source_updated_at: null, monitor: true },
  { id: 'pt_budget_2023_crypto', jurisdiction: 'portugal', authority: 'Diário da República', title: 'Lei n.º 24-D/2022 · regime fiscal de criptoativos', url: 'https://diariodarepublica.pt/dr/detalhe/lei/24-d-2022-205557192', source_type: 'PRIMARY_LAW', binding_level: 'PRIMARY_LAW', source_updated_at: '2022-12-30' },
  ...
  { id: 'us_1099_da', jurisdiction: 'estados-unidos', authority: 'Internal Revenue Service', title: 'Instructions for Form 1099-DA (2026)', url: 'https://www.irs.gov/instructions/i1099da', source_type: 'FORM_INSTRUCTIONS', binding_level: 'OFFICIAL_GUIDANCE', source_updated_at: null },
  ...
  { id: 'co_concept_2026', jurisdiction: 'colombia', authority: 'DIAN', title: 'Concepto 5146 de 2026 · CARF y valoración', url: 'https://normograma.dian.gov.co/dian/compilacion/docs/oficio_dian_5146_2026.htm', source_type: 'ADMINISTRATIVE_DOCTRINE', binding_level: 'OFFICIAL_INTERPRETATION', source_updated_at: '2026-03-31' },
  ...
  { id: 'cl_f1964', jurisdiction: 'chile', authority: 'Servicio de Impuestos Internos', title: 'Resolución Exenta SII n.º 114 · Formulario 1964', url: 'https://www.sii.cl/normativa_legislacion/resoluciones/2025/reso114.pdf', source_type: 'OFFICIAL_RESOLUTION', binding_level: 'PRIMARY_LAW', source_updated_at: '2025-08-26' },
...
const CHANGE_SIGNALS = Object.freeze([
  { id: 'co-carf-2026', official_date: '2026-03-31', official_date_kind: 'SIGNED', jurisdiction: 'colombia', title: 'DIAN fija su criterio sobre CARF y valoración de criptoactivos', impact: 'El Concepto 5146 confirma el reporte CARF para proveedores y mantiene las reglas tributarias generales para valor patrimonial, coste fiscal e ingresos por enajenación.', source_ids: ['co_concept_2026'] },
  { id: 'us-1099da-basis-2026', official_date: '2026-01-01', official_date_kind: 'APPLICATION_START', jurisdiction: 'estados-unidos', title: 'Form 1099-DA incorpora el reporte obligatorio de base', impact: 'Los brokers estadounidenses deben informar la base de determinados activos cubiertos en ventas realizadas desde esta fecha; el reporte de ingresos brutos ya se aplica desde 2025.', source_ids: ['us_1099_da'] },
  { id: 'cl-f1964-2025', official_date: '2025-01-01', official_date_kind: 'APPLICATION_START', jurisdiction: 'chile', title: 'El Formulario 1964 empieza a cubrir operaciones con activos digitales', impact: 'La Resolución Exenta SII n.º 114 exige a los proveedores obligados informar operaciones y monederos de contribuyentes residentes; la primera declaración corresponde al año tributario 2026.', source_ids: ['cl_f1964'] },
  { id: 'pt-crypto-tax-2023', official_date: '2023-01-01', official_date_kind: 'ENTRY_INTO_FORCE', jurisdiction: 'portugal', title: 'Entra en vigor el régimen fiscal portugués para criptoactivos', impact: 'La Lei n.º 24-D/2022 incorpora al IRS la venta, la permuta diferida bajo condiciones, el periodo de 365 días y reglas específicas para minería.', source_ids: ['pt_budget_2023_crypto'] }
]);
...
export function validateFiscalSnapshot(snapshot) {
  ...
  const officialDateKinds = new Set(['ENTRY_INTO_FORCE', 'APPLICATION_START', 'SIGNED']);
  for (const signal of snapshot.change_signals || []) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(signal.official_date || '')) throw new Error(`Invalid official fiscal date: ${signal.id}`);
    if (!officialDateKinds.has(signal.official_date_kind)) throw new Error(`Invalid official fiscal date kind: ${signal.id}`);
    if ('date' in signal || 'detected_at' in signal || 'generated_at' in signal) throw new Error(`Internal timestamp exposed as fiscal date: ${signal.id}`);
    if (!signal.source_ids?.length || !signal.source_ids.every((sourceId) => sourceIds.has(sourceId))) throw new Error(`Unsourced fiscal change signal: ${signal.id}`);
  }
