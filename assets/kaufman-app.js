(() => {
  ...
  function decisionCloseMarkup(page){
    if(page==='mineria'||page==='home'||page==='fiscal')return '';
    const context={
  ...
  function walletLandscapeMarkup(){
    const groups=[
      {code:'01',label:'Conectadas',title:'Wallets calientes',note:'Uso frecuente desde móvil, navegador u ordenador.',items:[
        {name:'MetaMask',fit:'EVM, Bitcoin, Solana y conexión con dApps',signal:'Compatibilidad Web3',url:'https://metamask.io/'},
        {name:'Trust Wallet',fit:'Móvil multichain y activos muy diversos',signal:'100+ redes · 200 M usuarios declarados',url:'https://trustwallet.com/'},
        {name:'Phantom',fit:'Solana y uso multichain de consumo',signal:'15 M+ usuarios declarados',url:'https://phantom.com/about'},
        {name:'Rabby',fit:'Operación DeFi en redes EVM',signal:'Simulación antes de firmar',url:'https://rabby.io/'},
        {name:'Electrum',fit:'Bitcoin en escritorio, multisig y watch-only',signal:'Código abierto · SPV',url:'https://electrum.org/'}
      ]},
      {code:'02',label:'Firma aislada',title:'Wallets frías',note:'El dispositivo conserva la clave y confirma fuera del equipo conectado.',items:[
        {name:'NGRAVE ZERO',fit:'Custodia prolongada y firma mediante QR',signal:'Mayor aislamiento',url:'https://ngrave.io/en/page/backup/zero/'},
        {name:'Ledger Flex',fit:'Amplia compatibilidad y uso multichain',signal:'8 M+ signers vendidos por Ledger',url:'https://www.ledger.com/academy/topics/crypto/which-ledger-signer-should-i-buy'},
        {name:'Trezor Safe 5',fit:'Pantalla táctil, código abierto y multishare',signal:'Secure Element EAL6+',url:'https://trezor.io/trezor-safe-5'},
        {name:'BitBox02',fit:'Bitcoin o multichain con interfaz contenida',signal:'Firmware reproducible · doble chip',url:'https://bitbox.swiss/bitbox02/security-features/'},
        {name:'COLDCARD Q',fit:'Bitcoin avanzado, PSBT y multifirma',signal:'QR · doble Secure Element',url:'https://coldcard.com/q'}
      ]},
      {code:'03',label:'Varios firmantes',title:'Tesorería y custodia',note:'Políticas, permisos y recuperación para equipos y entidades.',items:[
        {name:'Safe',fit:'Multifirma y cuentas inteligentes onchain',signal:'Autocustodia programable',url:'https://www.safe.global/security'},
        {name:'Fireblocks',fit:'Tesorería institucional y flujos de aprobación',signal:'MPC · motor de políticas',url:'https://www.fireblocks.com/'},
        {name:'BitGo',fit:'Custodia y autocustodia institucional',signal:'Multisig o MPC',url:'https://developers.bitgo.com/docs/wallet-types'},
        {name:'Copper',fit:'Custodia conectada a mercados',signal:'MPC · liquidación extrabursátil',url:'https://copper.co/en-us/solutions/hold'},
        {name:'Coinbase Prime',fit:'Custodia regulada, trading y financiación',signal:'Custodia cualificada',url:'https://help.coinbase.com/en/prime/prime-custody/introduction'}
      ]}
    ];
    return `<section class="kf-wallet-landscape" aria-labelledby="wallet-types-title"><header><div><p class="kf-kicker">Tipos y uso</p><h2 id="wallet-types-title">Tipos de wallet según su uso.</h2></div><p>No existe una mejor para todo. La elección cambia según frecuencia de uso, aislamiento y número de personas que deben autorizar.</p></header><div class="kf-wallet-groups">${groups.map((group)=>`<section><div class="kf-wallet-group-head"><span>${group.code}</span><div><small>${group.label}</small><h3>${group.title}</h3><p>${group.note}</p></div></div><ol>${group.items.map((item,index)=>`<li><span>${String(index+1).padStart(2,'0')}</span><a href="${item.url}" target="_blank" rel="noopener noreferrer"><strong>${item.name}</strong><p>${item.fit}</p><small>${item.signal} ↗</small></a></li>`).join('')}</ol></section>`).join('')}</div><p class="kf-wallet-landscape-note"><strong>Selección, no podio:</strong> adopción pública, especialización y arquitectura no son la misma métrica. «Fría» depende también del uso: un signer conectado habitualmente a dApps aumenta su exposición. Antes de mover fondos deben verificarse compatibilidad, firmware, procedencia y recuperación.</p></section>`;
  }
  ...
  function walletIntelligenceMarkup(){
    ...
    return `<section class="kf-wallet-intel" aria-labelledby="wallet-compare-title"><div class="kf-wallet-heading"><div><p class="kf-kicker">Comparativa de hardware</p><h2 id="wallet-compare-title">Tres opciones, cinco diferencias.</h2></div><p>NGRAVE prioriza aislamiento. Ledger prioriza integraciones. Trezor combina hardware certificado y código abierto.</p></div><div class="kf-wallet-matrix"><div class="kf-wallet-products"><span></span>${devices.map((device,index)=>`<a class="${index===0?'featured':''}" href="${profileUrl('wallets',device.id)}"><small>${escapeHtml(device.label)}</small><strong>${escapeHtml(device.name)}</strong><i>Ver ficha →</i></a>`).join('')}</div>${criterion('Uso principal','best')}${criterion('Conexión','isolation')}${criterion('Certificación','certification')}${criterion('Confirmación','verification')}${criterion('Recuperación','recovery')}</div><p class="kf-wallet-cert-note"><strong>Cómo leer EAL:</strong> mide el alcance evaluado de un componente o sistema; no es una puntuación total de seguridad ni sustituye la gestión de copias y firmas.</p><nav class="kf-wallet-sources" aria-label="Fuentes oficiales de la comparación">${devices.map((device)=>`<a href="${device.source}" target="_blank" rel="noopener noreferrer">${escapeHtml(device.name)} · fuente oficial ↗</a>`).join('')}</nav></section>`;
  }
  ...
  function renderDirectory(type){
    ...
    if(type==='wallets')return `<main class="kf-main" id="main-content">${walletHeroMarkup()}<section class="kf-section kf-wallet-page"><div class="kf-container">${walletLandscapeMarkup()}${walletIntelligenceMarkup()}</div></section></main>`;
  ...
  function fiscalServiceIcon(type){
    const paths={
      operation:'<path d="M7 3h10v18l-2-1.4-3 1.4-3-1.4L7 21V3Z"/><path d="M10 8h4M10 12h4M10 16h2"/>',
      obligations:'<path d="M6 3h9l3 3v15H6V3Z"/><path d="M15 3v4h4M9 11h6M9 15h6"/><path d="m9 18 1.5 1.5L14 17"/>',
      evidence:'<circle cx="10" cy="10" r="5.5"/><path d="m14 14 6 6M8 10l1.5 1.5L12.5 8"/>',
      company:'<path d="M4 21h16M6 21V7l6-4 6 4v14M9 9h2M13 9h2M9 13h2M13 13h2M10 21v-4h4v4"/>',
      jurisdiction:'<circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18"/>'
    };
    return `<i class="kf-fiscal-service-icon" data-fiscal-icon="${type}" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.35" stroke-linecap="round" stroke-linejoin="round">${paths[type]||''}</svg></i>`;
  }
  ...
  function renderFiscal(){
    return `<main class="kf-main" id="main-content">${fiscalHeroMarkup()}
      <section class="kf-section kf-fiscal-live" data-fiscal-dashboard><div class="kf-container">
        <section class="kf-fiscal-changes-primary" aria-labelledby="fiscal-changes-title"><div class="kf-section-head kf-fiscal-section-head"><div><p class="kf-kicker">Fechas oficiales</p><h2 class="kf-title small" id="fiscal-changes-title">Cambios con efecto fiscal.</h2></div></div><div class="kf-fiscal-change-list" data-fiscal-changes><div class="kf-live-empty">Esperando cambios verificados…</div></div></section>

        <section class="kf-fiscal-service" aria-labelledby="fiscal-service-title"><header><div><p class="kf-kicker">Fiscalidad blockchain · pagar, declarar y demostrar</p><h2 id="fiscal-service-title">¿Sabes qué debes pagar, declarar y poder demostrar?</h2></div><p>Partimos de una operación concreta, tu residencia y la evidencia disponible. Calculamos lo verificable y señalamos los supuestos, obligaciones y vacíos que pueden cambiar el resultado.</p></header><ol class="kf-fiscal-service-questions"><li><a href="/contacto/?asunto=fiscal-blockchain&amp;tema=importe"><span>01 · Operación</span>${fiscalServiceIcon('operation')}<strong>¿Cuánto puede costarte fiscalmente vender, permutar, cobrar o minar —incluso sin retirar euros?</strong><em>Calcular mi impacto fiscal →</em></a></li><li><a href="/contacto/?asunto=fiscal-blockchain&amp;tema=reporting"><span>02 · Obligaciones</span>${fiscalServiceIcon('obligations')}<strong>¿Qué obligaciones de declaración y registro activa tu forma de operar con exchanges, custodios, wallets o medios de pago?</strong><em>Identificar mis obligaciones →</em></a></li><li><a href="/contacto/?asunto=fiscal-blockchain&amp;tema=evidencia"><span>03 · Evidencia</span>${fiscalServiceIcon('evidence')}<strong>Si hoy tuvieras que justificar una operación, ¿podrías demostrar su coste, titularidad y origen de los fondos?</strong><em>Verificar mi evidencia →</em></a></li><li><a href="/contacto/?asunto=fiscal-blockchain&amp;tema=empresa"><span>04 · Empresa</span>${fiscalServiceIcon('company')}<strong>¿Tiene tu empresa resueltas la contabilización, valoración, custodia y tributación de sus criptoactivos?</strong><em>Revisar mi operativa empresarial →</em></a></li><li><a href="/contacto/?asunto=fiscal-blockchain&amp;tema=jurisdiccion"><span>05 · Jurisdicción</span>${fiscalServiceIcon('jurisdiction')}<strong>¿Cambiar de jurisdicción mejora realmente tu situación después de aplicar residencia, sustancia y obligaciones de información?</strong><em>Comparar jurisdicciones →</em></a></li></ol><footer><a class="kf-button primary" href="/contacto/?asunto=fiscal-blockchain">Evaluar mi caso fiscal →</a><small>Describe la operación sin adjuntar declaraciones, documentos de identidad, claves ni datos sensibles. Primero delimitamos qué puede calcularse, qué evidencia falta y qué requiere revisión profesional.</small></footer></section>
  ...
  function renderContact(){
    ...
    const isFiscalService=params.get('asunto')==='fiscal-blockchain';
    const fiscalTopic=params.get('tema')||'';
  ...
  function renderFiscalChanges(data){
    ...
    if(!fiscalDataValid(data)){root.innerHTML='<div class="kf-live-empty">Cambios fiscales no disponibles.</div>';return}
    const sourceById=Object.fromEntries(data.sources.map((source)=>[source.id,source]));
    const dateKindLabels={ENTRY_INTO_FORCE:'Entrada en vigor',APPLICATION_START:'Aplicación oficial',SIGNED:'Firmado'};
    const signals=[...(data.change_signals||[])].sort((a,b)=>String(b.official_date).localeCompare(String(a.official_date)));
    root.innerHTML=signals.map((signal,index)=>{const jurisdiction=data.jurisdictions.find((row)=>row.id===signal.jurisdiction),source=sourceById[signal.source_ids[0]],dateLabel=dateKindLabels[signal.official_date_kind]||'Fecha oficial';return `<article><span>${String(index+1).padStart(2,'0')}</span><time datetime="${escapeHtml(signal.official_date)}"><small>${dateLabel}</small><strong>${new Intl.DateTimeFormat('es-ES',{day:'2-digit',month:'short',year:'numeric'}).format(new Date(signal.official_date+'T12:00:00Z'))}</strong></time><div><small>${escapeHtml(jurisdiction?.name||signal.jurisdiction)}</small><h3>${escapeHtml(signal.title)}</h3><p>${escapeHtml(signal.impact)}</p></div>${source?`<a href="${safeExternalUrl(source.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(source.authority)} ↗</a>`:''}</article>`}).join('');
  }
  ...
  const commercialPages=!['home','regulacion','wallets','fiscal','contacto','aviso','privacidad','cookies','terminos','retirado'].includes(page);
